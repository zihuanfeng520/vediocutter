...
class VideoCutter(QMainWindow):
    ...
    def initUI(self):
        ...
        # --- 刪除當前影片按鈕 ---
        # ==========================================
        self.delete_file_btn = QPushButton("🗑️ 刪除當前影片")
        ...
        self.delete_file_btn.clicked.connect(self.delete_current_video)
        right_panel_layout.addWidget(self.delete_file_btn)
        # ==========================================
    ...
    def delete_current_video(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            # 【關鍵步驟】：讓播放器停止並清空來源，釋放作業系統對檔案的佔用鎖定
            self.media_player.stop()
            self.media_player.setSource(QUrl())
            # 強制 Qt 立即處理上述清空操作，確保檔案句柄 (Handle) 已經放開
            QApplication.processEvents()
            
            try:
                os.remove(self.video_path)
                QMessageBox.information(self, "成功", "影片已成功刪除。")
                
                # 刪除成功後，重置所有 UI 狀態
                self.video_path = None
                self.file_text_label.setText("點擊選擇影片\n或將檔案拖放至此")
                self.file_text_label.setStyleSheet("color: #ccc; font-weight: bold; font-size: 16px;")
                self.file_info_box.setVisible(False)
                self.delete_file_btn.setVisible(False)
                ...
                self.end_slider.setEnabled(False)
                self.process_btn.setEnabled(False)
                self.preview_speed_check.setEnabled(False)
                self.jump_start_btn.setEnabled(False)
                self.jump_end_btn.setEnabled(False)
                self.start_back_btn.setEnabled(False)
                self.start_fwd_btn.setEnabled(False)
                self.end_back_btn.setEnabled(False)
                self.end_fwd_btn.setEnabled(False)
                
                self.duration_label.setText("00:00:00")
                self.current_time_label.setText("00:00:00.000")
                self.range_info_label.setText("總長度: 00:00:00")
                self.size_estimate_label.setText("預估大小: -")
    ...
    def load_video(self, file_path):
        ...
        self.delete_file_btn.setVisible(True) # 載入影片時，顯示刪除按鈕
    ...
    def process_video(self):
        ...
        # 1. 先針對「原始影片所在資料夾」算一個不重複的預設檔名
        # 這樣存檔視窗跳出來時，預設顯示的名稱就會是最準確的
        default_name = f"{base_name}_cut.{output_format}"
        default_path = os.path.join(base_dir, default_name)
        
        counter = 1
        while os.path.exists(default_path):
            default_name = f"{base_name}_cut_{counter}.{output_format}"
            default_path = os.path.join(base_dir, default_name)
            counter += 1
            
        # 2. 開啟存檔對話框，並且【強制關閉 Windows 內建的覆蓋警告】(DontConfirmOverwrite)
        output_path, _ = QFileDialog.getSaveFileName(
            self, 
            "儲存影片", 
            default_path, 
            f"{output_format.upper()} 檔案 (*.{output_format})",
        ...
        # 3. 拿到你最終決定的存檔路徑後 (例如你切換到了桌面)
        # 再針對那個新資料夾做最後一次檢查，確保絕對不覆蓋
        ...
        while os.path.exists(final_path):
            final_path = os.path.join(final_dir, f"{final_basename}_{post_counter}{final_ext}")
            post_counter += 1
            
        # 把檢查完畢的絕對安全路徑，交給後面的 ffmpeg 執行
